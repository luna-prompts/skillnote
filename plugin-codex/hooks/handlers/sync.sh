#!/bin/bash
# SkillNote Sync (Codex) — SessionStart hook
# Syncs the active collection's skills into PROJECT/.codex/skills/ with full
# frontmatter. Manages create/update/delete via a manifest. Offline-first
# (silent fail using cached skills). Codex runs hooks with the session cwd as
# the working directory, so PROJECT_DIR is $PWD.
...
# Always project-level — never write to global ~/.codex/skills/
SKILLS_DIR="${PROJECT_DIR}/.codex/skills"

# Use the plugin data dir for the manifest if Codex provides one, else
# keep it alongside the synced skills.
if [ -n "$PLUGIN_DATA" ]; then
    MANIFEST_DIR="$PLUGIN_DATA"
else
...
# Skills the plugin already ships as bundled skills — don't create duplicate
# synced copies under .codex/skills/.
...
for skill in skills:
    ...
    if slug in plugin_provided:
        continue  # Plugin ships these as bundled skills

    # Prefix with skillnote- so all synced skills group together
...
# Check manifest (tracked skills) AND scan disk (catches orphans from before
# the manifest existed). Only ever touch skillnote-* directories.
...
# Output the branded summary (Codex logs hook stdout for the session)
if [ -n "$RESULT" ]; then
    echo "$RESULT"
fi
